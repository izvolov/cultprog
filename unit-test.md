...
2. Помогают понять, как устроена программная сущность, и как с ней обращаться
   ...
   Модульный тест — это, в том числе, и "активная документация". Эту "документацию" можно запустить
   и убедиться, что программная сущность соответствует требованиям, наложенным на неё.
...
3. Упрощают процесс переработки кода, затрагивающего программную сущность
   ...
   документацию", то об этом станет известно сразу же после запуска тестов. Более того, при грамотно
   написанных тестах будет сообщено, какое именно из требований перестало выполняться.
...
2. Автоматическими
   ...
   автоматизировать. Лучше всего встраивать выполнение тестов в сборку проекта. Запустить сборку он
   уж точно (ну, если не совсем угорелый) не забудет.
...
4. Независимыми
   ...
   Результаты прохождения тестов не должны зависеть от того, какое подмножество тестов запускается,
   и в каком порядке.
...
4. Трёхчастное произведение
   ...
   Подготовка исходных данных и действие могут быть очень простыми или не выражены в коде явно. Тем
   не менее, они всегда есть.
...
2. Ничего лишнего
   ...
   Если подготовка исходных данных или проверка полученных результатов сложно читаются или занимают
   слишком много места, то нужно декомпозировать тест так же, как и обычную функцию — вынести всё
   лишнее в отдельные сущности, а в тесте оставить только суть.
...
5. Строгое разграничение ответственности
...
3. Особенности интерфейса
   ...
   2. Функция ```my::sin``` принимает на вход один аргумент — число с плавающей точкой, и возвращает
      тот же тип.
